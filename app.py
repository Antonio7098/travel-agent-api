...
# ==============================================================================
# 1. TOOL SCHEMAS (The "Blueprints" for the AI)
# ==============================================================================
...
class CreateTripInput(BaseModel):
    tripName: str = Field(description="The name for the new trip, for example 'Summer Holiday' or 'Work Conference'.")
    location: str = Field(description="The primary destination for the trip, for example 'Paris, France'.")
    startDate: str = Field(description="The start date in 'YYYY-MM-DD' format.")
    endDate: str = Field(description="The end date in 'YYYY-MM-DD' format.")

class AddFlightInput(BaseModel):
    tripId: str = Field(description="The unique ID of the trip to add this flight to.")
    airline: str = Field(description="The name of the airline, e.g., 'British Airways'.")
    flightNumber: str = Field(description="The flight number, e.g., 'BA283'.")
    departureTime: str = Field(description="The departure date and time in 'YYYY-MM-DDTHH:MM' format.")
    arrivalTime: str = Field(description="The arrival date and time in 'YYYY-MM-DDTHH:MM' format.")
...
@tool(args_schema=GetItineraryInput)
def get_trip_itinerary(tripId: str, firebase_token: str) -> str:
    """Retrieves a day-by-day itinerary for a specific trip."""
    print(f"--- Calling Tool: get_trip_itinerary for tripId: {tripId} ---")
    try:
        headers = {"Authorization": f"Bearer {firebase_token}"}
        # Remember to update localhost to your deployed backend URL when live
        response = requests.get(f"http://localhost:3001/api/{tripId}/itinerary", headers=headers, timeout=15)
        response.raise_for_status()
        return json.dumps(response.json())
    except requests.exceptions.RequestException as e:
        return f"Error: Could not retrieve the itinerary. Server said: {e}"
...
def create_trip(tripName: str, location: str, startDate: str, endDate: str, firebase_token: str) -> str:
    ...
    try:
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {firebase_token}"}
        payload = {"tripName": tripName, "location": location, "startDate": startDate, "endDate": endDate}
        response = requests.post("http://localhost:3001/api/trips", headers=headers, json=payload, timeout=10)
        response.raise_for_status()
        trip_id = response.json().get('id', 'Unknown ID')
        return f"Success! The trip to {location} has been created with ID: {trip_id}."
    except requests.exceptions.RequestException as e:
        return f"Error: Could not create the trip. Server said: {e}"
...
def add_flight_to_trip(tripId: str, airline: str, flightNumber: str, departureTime: str, arrivalTime: str, firebase_token: str) -> str:
    ...
    try:
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {firebase_token}"}
        payload = {"airline": airline, "flightNumber": flightNumber, "departureTime": departureTime, "arrivalTime": arrivalTime}
        response = requests.post(f"http://localhost:3001/api/trips/{tripId}/flights", headers=headers, json=payload, timeout=10)
        response.raise_for_status()
        return f"Successfully added flight {flightNumber} to trip {tripId}."
    except requests.exceptions.RequestException as e:
        return f"Error: Could not add the flight. Server said: {e}"
        
@tool(args_schema=FlightCheckInput)
def check_flights(tripId: str, firebase_token: str) -> str:
    """Checks for flight details for a specific trip by its ID."""
    print(f"--- Calling Tool: check_flights for tripId: {tripId} ---")
    try:
        headers = { "Content-Type": "application/json", "Authorization": f"Bearer {firebase_token}" }
        response = requests.get(f"http://localhost:3001/api/trips/{tripId}/flights", headers=headers, timeout=10)
        response.raise_for_status()
        return json.dumps(response.json())
    except requests.exceptions.RequestException as e:
        return f"Error: Could not retrieve flight information. Server said: {str(e)}"

# ... Add your other tool functions here (check_accom, add_accom, etc.)


# ==============================================================================
# 3. DYNAMIC PROMPT GENERATOR
# ==============================================================================
def create_travel_prompt(trips: list = None) -> str:
    current_date_str = datetime.now(timezone.utc).strftime("%A, %B %d, %Y")
    persona_and_rules = f"""
You are a world-class, friendly, and helpful travel agent assistant named Omni. Your primary goal is to assist users with their travel plans using the tools and information available. Maintain an enthusiastic and proactive tone.

**Initial Message Rule:**
Your first message MUST be a brief, welcoming greeting. Do not mention trip details unless the user asks. Example: "Hello! I'm Omni, your travel assistant. How can I help today?"

--------------------
**TOOL USAGE RULES - VERY IMPORTANT:**
1. When you need to use a tool like `check_flights` or `get_trip_itinerary`, you MUST find the correct `tripId` from the "User's Current Trip Information" section.
2. Look for the trip the user is asking about (e.g., "my trip to Rome").
3. Find that trip in the list and extract the value labeled 'ID'.
4. Use that `tripId` value for the tool call.
5. **DO NOT ask the user for the `tripId` or `firebase_token`. You have all the information you need.**
--------------------

**Context Rules:**
Use the information below to answer questions. Today's date is {current_date_str}.
"""
    if trips:
        trips.sort(key=lambda x: x.get('startDate', {}).get('seconds', float('inf')))
        trip_details_list = [f"- Trip to {t.get('location','N/A')} with ID '{t.get('id','N/A')}' (named '{t.get('tripName','No Name')}') from {datetime.fromtimestamp(t['startDate']['seconds'], tz=timezone.utc).strftime('%d %b %Y')} to {datetime.fromtimestamp(t['endDate']['seconds'], tz=timezone.utc).strftime('%d %b %Y')}." for t in trips if 'startDate' in t and 'endDate' in t]
        trip_details = "\n".join(trip_details_list)
        context_block = f"--- User's Current Trip Information ---\n{trip_details}\n---"
    else:
        context_block = "--- User's Current Trip Information ---\nNo trip information was provided for this user.\n---"
...
class Agent:
    def __init__(self, model: ChatVertexAI, tools: list):
        ...
        graph.add_node("llm", self.call_gemini)
        graph.add_node("action", self.take_action)
        graph.add_conditional_edges("llm", self.exists_action, {"action": "action", END: END})
        graph.add_edge("action", "llm")
        graph.set_entry_point("llm")
        self.runnable = graph.compile()
    ...
    def call_gemini(self, state: AgentState, config: RunnableConfig) -> dict:
        print("\n--- Calling Gemini ---")
        trip_data = config["configurable"].get("trip_data", [])
        system_prompt = create_travel_prompt(trips=trip_data)
        
        # ✅ FIX: Filter out any messages that might have empty content
        valid_messages = [m for m in state['messages'] if m.content]
        
        messages = [SystemMessage(content=system_prompt)] + valid_messages
        
        print("✅ System prompt created with user's trip data.")
        print(f"Invoking model with {len(messages)} total messages...")
        response = self.model.invoke(messages)
        print(f"✅ Model responded. Content: '{response.content[:100]}...'")
        return {'messages': [response]}
...
model = ChatVertexAI(model_name='gemini-2.0-flash-lite-001', project='nc-travel-462414', location='us-central1')
...
def chat_with_agent():
    try:
        ...
        agent_input = {"messages": messages}
        agent_config = {"configurable": {"firebase_token": firebase_token, "trip_data": trip_data}}
